...
scatfiles = dir(fullfile([folder,'/Scatter/'],'*.mat'));
...
%%
if Uflood
%    Uniform = load('/media/ashley/My Passport/DOI_Image_Proj/UXYCorrected/XY/London/U/20191216/Corr_L20191216_Tc99m_Flood_0p75MBq_1350hrs_2min_01.mat');
%    Uniform = load('/media/ashley/My Passport/DOI_Image_Proj/UXYCorrected/XY/London/U/20191001/Corr_L20191001_Tc99m_ps_Flood_1p3Mbq_1135time_AllNode_10mins_01.mat');
    Uniform = load('/media/ashley/My Passport/DOI_Image_Proj/UXYCorrected/XY/London/U/20191015/Corr_L20191015_PS_Flood_Tc99m_noColl_1p88Mbq_1155time_5min_01.mat');
...
scatfilename = scatfiles(i).name;
scatfilepath = [scatfiles(i).folder,'/'];

scatdata = open([scatfilepath,scatfilename]);
    try 
...
for j = 1:num_nod
    ...
    if ~isempty(data.NodeData{j})
       ...
       ScatData = scatdata.NodeData{j}; 
    else
        continue
    end 
    NodeData = squeeze(NodeData) - squeeze(ScatData);
    NodeData(NodeData < 0) = 0;
    NodeData(:,1:18) = 0;
    NodeData(1:10,:) = 0;
    NodeData(:,502:end) = 0;
    NodeData(250:end,:) = 0;
...
figure, for i = 1:num_nod, subplot(4,5,i), imagesc(squeeze(UCorrData(:,:,i))'), daspect([1 1 1]), title(['Det ',int2str(i)]),end


fn = ['/media/ashley/My Passport/DOI_Image_Proj/UXYCorrected/XY/London/ScatC',filename(10:end)];
if Uflood
